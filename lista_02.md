...
B) O código avalia a expressão booleana, imprime o resultado `true`, calcula a soma dos números de 1 a 5 e imprime o resultado no console. // **Alternativa escolhida**
...
B) A saída de calcularOrcamento() seria: 'Seu saldo é negativo de -1050.' e a de calcularOrcamento2() seria: 'Seu saldo é negativo de -100.' // **Alternativa escolhida**
...
B) Preload -> Create -> Update // **Alternativa escolhida**
...
B) Simular interações físicas realistas, como colisões e movimentos, em jogos 2D. // **Alternativa escolhida**
...
variável idade = prompt ('Insira sua idade');

IF (idade < 16) então:

	escreva ('Não pode votar!'); 

	ELSE IF (idade > OU = 16 E idade < 18) então:
...
Classe Retangulo extends FormaGeometrica: // Classe que herda atributos da classe FormaGeometrica
	Atributos:
	- cor; // herdou de FormaGeometrica
	- this.altura = altura;
	- this.largura = largura;

// Chama o método construtor da classe FormaGeomatrica para definir a cor do retangulo e define o valor da altura e largura

	método Construtor(cor, altura, largura) {

		Chama o construtor da classe base FormaGeometrica com cor
		Define o valor do atributo largura;
		Define o valor do atributo altura;


// Sobrescreve o método CalcularArea da classe FormaGeometrica definindo a area do retangulo
   	método CalcularArea() { 
        area = largura * altura
	}
...
// Chama o método construtor da classe FormaGeomatrica para definir a cor do círculo e define o valor do raio
...
// Sobrescreve o método CalcularArea da classe FormaGeometrica definindo a area do círculo
...
função corrida (){
	WHILE (distancia < distanciaTotal && tempo <= tempoMaximo) faça:
		velocidade = velocidadeInicial + aceleracao*tempo;

		IF (velocidade > velocidadeMaxima) então:
			velocidade = velocidadeMaxima;

		// Atualiza a distância percorrida
        	distancia = distancia + velocidade * 1; // Atualização a cada segundo

        	// Atualiza o tempo decorrido
        	tempo = tempo + 1;

	IF (distancia >= distanciaTotal) então:

		imprima  ('O carro terminou a corrida em' + tempo + 'minutos');

	ELSE IF (tempo > tempoMaximo) então:

		imprima ('A corrida não terminou no tempo máximo estipulado');
